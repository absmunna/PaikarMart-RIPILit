...
import { ShieldCheck, Upload, FileText, CheckCircle2, Clock, XCircle, X } from "lucide-react";
...
const DOC_KINDS = [
  { id: "nid" as const, label: "National ID" },
  { id: "passport" as const, label: "Passport" },
  { id: "business" as const, label: "Business Registration" },
];
...
export default function SellerVerificationPage() {
  ...
  const addMockDoc = (kind: "nid" | "passport" | "business") => {
    ...
    setDocs((prev) => [
      ...prev,
      { id: `doc_${Math.random().toString(36).slice(2, 8)}`, name: `${label}_${prev.length + 1}.pdf`, kind },
    ]);
  };
      ...
      <div className="container mx-auto px-4 py-8 flex flex-col gap-6 max-w-3xl">
        ...
        <GlassCard className="p-6 flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Select value={category} onValueChange={setCategory}>
              ...
              <SelectContent>
                {["Home & Lifestyle","Fashion","Electronics","Kitchen","Grocery","Services","Real Estate","Hotel & Tour"].map((c) => (
        ...
        <GlassCard className="p-6 flex flex-col gap-5">
          <div>
            ...
            <p className="text-sm text-white/50 mt-1">Upload UI is for preview — file picking is mocked.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {DOC_KINDS.map((d) => (
              <button key={d.id} type="button" onClick={() => addMockDoc(d.id)}
            ...
            <div className="flex flex-col gap-2">
              {docs.map((d) => (
                <div key={d.id} className="flex items-center gap-3 p-3 rounded-lg bg-white/5 border border-white/10">
                  ...
                  <Input value={d.name}
                    onChange={(e) => setDocs((prev) => prev.map((p) => p.id === d.id ? { ...p, name: e.target.value } : p))}
        ...
        <div className="flex justify-end">
          <Button onClick={onSubmit} disabled={status === "pending"} className="bg-primary hover:bg-primary/90 px-8">
