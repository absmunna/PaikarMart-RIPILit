    ...
    <Layout>
      <div className="container mx-auto px-4 py-8 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard icon={Eye} label="Views (7d)" value={totalViews.toLocaleString()} />
          <StatCard icon={ShoppingBag} label="Orders" value={String(totalOrders)} />
          <StatCard icon={MousePointerClick} label="Conversion" value={`${conversion}%`} />
          <StatCard icon={TrendingUp} label="Engagement" value="4.2%" />
        </div>
          ...
          <div className="h-[280px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={dailySales}>
                ...
                <Tooltip contentStyle={{ backgroundColor: "rgba(15,23,42,0.9)", borderColor: "rgba(255,255,255,0.1)", borderRadius: "8px" }} itemStyle={{ color: "white" }} />
                <Area type="monotone" dataKey="sales" stroke="hsl(var(--primary))" strokeWidth={3} fill="url(#anaSales)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </GlassCard>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="h-[240px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dailySales}>
                  ...
                  <XAxis dataKey="day" stroke="rgba(255,255,255,0.3)" fontSize={12} tickLine={false} axisLine={false} />
                  <YAxis stroke="rgba(255,255,255,0.3)" fontSize={12} tickLine={false} axisLine={false} />
                  <Tooltip contentStyle={{ backgroundColor: "rgba(15,23,42,0.9)", borderColor: "rgba(255,255,255,0.1)", borderRadius: "8px" }} itemStyle={{ color: "white" }} cursor={{ fill: "rgba(255,255,255,0.05)" }} />
          ...
          <GlassCard className="p-6">
            ...
            <div className="h-[180px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={trafficSources} innerRadius={45} outerRadius={70} paddingAngle={3} dataKey="value" stroke="none">
                ...
                return (
                  <div key={i} className="py-3 flex items-center gap-4">
                    ...
                    <div className="text-right text-xs text-white/60 w-24 shrink-0">
                      <div className="text-white text-sm font-semibold">{p.views.toLocaleString()}</div>
