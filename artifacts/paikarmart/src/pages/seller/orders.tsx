      ...
      <div className="container mx-auto px-4 py-8 flex flex-col gap-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-white/60 mt-1">Manage incoming orders and update fulfillment status.</p>
          </div>
                      ...
                      <TableRow key={o.id} className="border-white/10 hover:bg-white/5">
                        <TableCell>
                          <div className="flex items-center gap-3">
                            <img src={o.productImage} alt="" className="w-10 h-10 rounded object-cover border border-white/10 shrink-0" />
                            <div className="min-w-0">
                              <div className="text-sm text-white truncate">{o.productTitle}</div>
                              <div className="text-xs text-white/50">#{o.id} · qty {o.quantity}</div>
                            </div>
                          </div>
                        </TableCell>
                        ...
                        <TableCell className="text-primary font-semibold">{formatBDT(o.total)}</TableCell>
                        <TableCell>
                          <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${STATUS_STYLES[o.status]}`}>
                            {o.status}
                          </span>
                        </TableCell>
                        <TableCell className="text-right">
                          {nextStatus ? (
                            <Button size="sm" variant="outline" onClick={() => advance(o.id, o.status)}
                              className="bg-white/5 border-white/10 text-white hover:bg-white/10">
                              Mark {nextStatus} <ArrowRight className="w-3 h-3 ml-2" />
                            </Button>
                          ) : (
