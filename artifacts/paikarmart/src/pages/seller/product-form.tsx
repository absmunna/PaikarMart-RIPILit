...
export default function SellerProductFormPage() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    if (imageUrls.length === 0) { toast.error("Add at least one image URL"); return; }
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-8 flex flex-col gap-6 max-w-3xl">
        <Link href="/seller/products" className="inline-flex items-center gap-2 text-white/70 hover:text-white w-fit">
        ...
        <form onSubmit={onSubmit} className="flex flex-col gap-6">
          <GlassCard className="p-6 flex flex-col gap-5">
            <Field label="Title">
              <Input required value={form.title} onChange={(e) => update("title", e.target.value)}
                placeholder="Product or service name" className="bg-white/5 border-white/10 text-white" />
            </Field>

            <Field label="Description">
              <Textarea required value={form.description} onChange={(e) => update("description", e.target.value)}
                placeholder="Detailed description..." className="bg-white/5 border-white/10 text-white min-h-[120px]" />
            </Field>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <Field label="Price (৳)">
                <Input type="number" required min="0.01" step="0.01" value={form.price}
                  onChange={(e) => update("price", e.target.value)} className="bg-white/5 border-white/10 text-white" />
              </Field>
              <Field label="Stock">
                <Input type="number" required min="0" value={form.stock}
                  onChange={(e) => update("stock", e.target.value)} className="bg-white/5 border-white/10 text-white" />
              </Field>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <Field label="Listing Type">
                <Select value={form.type} onValueChange={(v: SellerType) => update("type", v)} required>
                  <SelectTrigger className="bg-white/5 border-white/10 text-white">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <Field label="Location">
              <Input required value={form.location} onChange={(e) => update("location", e.target.value)}
                placeholder="City, country" className="bg-white/5 border-white/10 text-white" />
            </Field>
          </GlassCard>
          ...
          <GlassCard className="p-6 flex flex-col gap-5">
            <div>
              ...
              <p className="text-xs text-white/50 mb-3">Paste image URLs separated by commas.</p>
              <div className="flex flex-wrap gap-3 mb-3">
                {previewImages.length === 0 ? (
                  ...
                  previewImages.map((url, i) => (
                    <img key={i} src={url} alt="" className="h-24 w-24 rounded-lg object-cover border border-white/10" />
                  ))
                )}
              </div>
              <Textarea value={form.images} onChange={(e) => update("images", e.target.value)}
                placeholder="https://image1.jpg, https://image2.jpg" className="bg-white/5 border-white/10 text-white" />
            </div>
            ...
            <Field label="Video URL (optional)">
              <div className="flex gap-3">
                ...
                <Input value={form.videoUrl} onChange={(e) => update("videoUrl", e.target.value)}
                  placeholder="https://video.mp4" className="bg-white/5 border-white/10 text-white flex-1" />
              </div>
            </Field>

            <Field label="Tags (comma separated)">
              <Input value={form.tags} onChange={(e) => update("tags", e.target.value)}
                placeholder="electronics, gadget, new" className="bg-white/5 border-white/10 text-white" />
            </Field>
          </GlassCard>

          <div className="flex justify-end gap-3">
            <Link href="/seller/products">
              <Button type="button" variant="ghost" className="text-white hover:bg-white/10">Cancel</Button>
            </Link>
