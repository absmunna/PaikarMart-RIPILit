      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="space-y-5">
                ...
                <Card className="border-white/8 shadow-sm">
                  <CardContent className="p-5">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="font-semibold text-white/85">Personal Information</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      {[
                        ...
                        const Icon = f.icon;
                        return (
                          <div key={f.label} className="flex items-center gap-3 p-3 bg-white/3 rounded-xl">
                            <Icon className="h-4 w-4 text-green-600 shrink-0" />
                            <div>
                              <p className="text-[10px] text-white/45 uppercase tracking-wide">{f.label}</p>
                              <p className="font-medium text-white/85">{f.value}</p>
                            </div>
                          </div>
                        );
                      })}
                ...
                {orders.length > 0 && (
                  <Card className="border-white/8 shadow-sm">
                    <CardContent className="p-5">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="font-semibold text-white/85">Recent Orders</h3>
                        ...
                        {orders.slice(0, 3).map((order: any) => {
                          ...
                          return (
                            <Link href={`/orders/${order.id}`} key={order.id}>
                              <div className="flex items-center justify-between p-3 rounded-xl hover:bg-white/3 transition-colors cursor-pointer">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-xs text-white/45">{new Date(order.createdAt).toLocaleDateString()}</p>
                                  </div>
                                </div>
                                <div className="flex items-center gap-2">
                                  <span className="font-bold text-white/85 text-sm">৳{order.totalAmount.toLocaleString()}</span>
            ...
            {activeTab === "orders" && (
              <div>
                <h2 className="text-xl font-bold text-white mb-5">My Orders</h2>
                {orders.length === 0 ? (
                  <Card className="border-white/8 shadow-sm">
                    <CardContent className="py-16 text-center">
                      ...
                      <p className="text-white/45 text-sm mb-6">Browse products and place your first order</p>
                  ...
                  <div className="space-y-4">
                    {orders.map((order: any) => {
                      ...
                      return (
                        <Link href={`/orders/${order.id}`} key={order.id}>
                          <Card className="border-white/8 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                            <CardContent className="p-5">
                              ...
                              <div className="flex items-center justify-between">
                                <p className="text-sm text-white/60">{(order.items as any[]).length} item(s)</p>
                                <p className="font-bold text-green-700">৳{order.totalAmount.toLocaleString()}</p>
                              </div>
                              {order.trackingCode && (
                                <div className="mt-2 text-xs text-white/45 flex items-center gap-1">
            ...
            {activeTab === "wishlist" && (
              <div>
                ...
                <Card className="border-white/8 shadow-sm">
                  <CardContent className="py-16 text-center">
                    <Heart className="h-16 w-16 text-white/15 mx-auto mb-4" />
                    <h3 className="text-lg font-semibold text-white/70 mb-2">Your wishlist is empty</h3>
                    <p className="text-white/45 text-sm mb-6">Save products you love for later</p>
            ...
            {activeTab === "reviews" && (
              <div>
                <h2 className="text-xl font-bold text-white mb-5">My Reviews</h2>
                <Card className="border-white/8 shadow-sm">
                  <CardContent className="py-16 text-center">
                    <Star className="h-16 w-16 text-white/15 mx-auto mb-4" />
                    <h3 className="text-lg font-semibold text-white/70 mb-2">No reviews yet</h3>
                    <p className="text-white/45 text-sm">Purchase and receive products to leave reviews</p>
                  </CardContent>
                </Card>
              </div>
            )}
            ...
            {activeTab === "wallet" && (
              <div>
                <h2 className="text-xl font-bold text-white mb-5">My Wallet</h2>
                <Card className="border-white/8 shadow-sm">
                  <CardContent className="py-12 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-white/70 mb-2">Wallet System</h3>
                    <Badge className="bg-orange-100 text-orange-600 border-orange-200 mb-4">Coming Soon</Badge>
                    <p className="text-white/45 text-sm max-w-sm mx-auto mb-6">Earn rewards on every purchase. Track your balance, rewards, and investment value.</p>
            ...
            {activeTab === "settings" && (
              <div>
                ...
                <div className="space-y-5">
                    ...
                    <CardContent className="p-6">
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <Input value={name} onChange={e => setName(e.target.value)} disabled={!editMode} className={editMode ? "border-green-300 focus:border-green-500" : "bg-white/3"} />
                        </div>
                        <div>
                          <Label className="text-sm font-medium text-white/70 mb-1.5 block">Phone Number</Label>
                          <Input value={phone} onChange={e => setPhone(e.target.value)} disabled={!editMode} className={editMode ? "border-green-300 focus:border-green-500" : "bg-white/3"} />
                        </div>
                        <div>
                          <Label className="text-sm font-medium text-white/70 mb-1.5 block">Email Address</Label>
                          <Input type="email" value={emailVal} onChange={e => setEmailVal(e.target.value)} disabled={!editMode} placeholder="Add your email" className={editMode ? "border-green-300 focus:border-green-500" : "bg-white/3"} />
                        </div>
                        {editMode && (
                  ...
                  <Card className="border-white/8 shadow-sm">
                    <CardContent className="p-6">
                      <h3 className="font-semibold text-white/85 mb-4 flex items-center gap-2"><Bell className="h-4 w-4 text-green-600" /> Notifications</h3>
                      <div className="space-y-3">
                        {[
                          ...
                          <div key={n.label} className="flex items-center justify-between p-3 rounded-xl bg-white/3">
                            <div>
                  ...
                  <Card className="border-white/8 shadow-sm">
                    <CardContent className="p-6">
                      <h3 className="font-semibold text-white/85 mb-4 flex items-center gap-2"><Lock className="h-4 w-4 text-green-600" /> Security</h3>
                      <Button variant="outline" className="w-full border-white/10 text-white/60 hover:bg-white/3" onClick={() => window.location.href = "/login"}>
                        Change Password
                      </Button>
                    </CardContent>
                  </Card>
